...
- Refreshed artifacts committed in the repository:
  ...
  - `artifacts/nightly/dependency-audit/reviewed-production-exceptions.md`
...
- The targeted parser/security closeout from `ASE-35` removes those rows from the parity-blocking backlog:
  - `prop-09`, `parser-01`, `parser-04`, `parser-08`, `parser-11`, `sec-03`, and `sec-05` now have direct regression evidence
  - `parser-10` is no longer treated as hidden implementation backlog; it is now explicit `drift-11` parser-shape contract drift
- The production dependency and license decisions were re-reviewed for `ASE-40`:
  - the `entities` `BSD-2-Clause` exception was removed by allowlisting `BSD-2-Clause`
  - the `khroma` metadata exception now points to repo-hosted MIT-license evidence and is explicitly non-blocking
  - the remaining `lodash-es` advisory via `mermaid` is now documented as the only dependency-policy exception that still blocks the first trusted release
...
| Closed parser and security blocker set (`prop-09`, `parser-01`, `parser-04`, `parser-08`, `parser-11`, `sec-03`, `sec-05`)                                                                                                                         | `closed`                               | `docs/parity-matrix.md`, `docs/parity-contract.md`, `tests/contracts/parser-blocks.spec.ts`, `tests/ported/remend/inline-code.test.ts`, `tests/ported/remend/html-tags.test.ts`, `tests/ported/streamdown/security/url-policy*.test.ts`, `tests/ported/streamdown/security/markdown-filtering*.test.ts`, and browser footnote/CJK/core-prop evidence | These rows are no longer part of the parity-blocking backlog.                                                                                                                                                                                                    |
| Accepted parser-shape drift (`parser-10`)                                                                                                                                                                                                          | `accepted drift`                       | `docs/parity-matrix.md`, `docs/parity-contract.md`, and `tests/ported/streamdown/rendering/footnotes.svelte.test.ts`                                                                                                                                                                                                                                 | The parser still stores footnote definitions differently from the frozen IR, but that difference is now explicit contract drift rather than hidden release-blocking debt.                                                                                        |
| Remaining rendering and interaction backlog (`prop-08`, `prop-19`, `prop-21` through `prop-24`, `render-05`, `render-10`, `prop-05`, `prop-06`, `prop-07`, `prop-10`, `prop-18`, `interact-02`, `interact-04`, `interact-08`)                      | `still blocking parity-backed release` | `docs/parity-matrix.md` categorized unresolved backlog                                                                                                                                                                                                                                                                                               | The compatibility contract still documents material open parity work rather than a fully-closed frozen target.                                                                                                                                                   |
...
| Reviewed dependency and license exceptions                                                                                                                                                                                                         | `still blocking first trusted release` | local `pnpm verify:dependency-policy` passed with `1` approved production `high` advisory, `19` approved development `high` advisories, and `1` approved production license metadata exception, and `artifacts/nightly/dependency-audit/reviewed-production-exceptions.md` records the `2026-04-04` production decisions                             | The temporary `entities` exception is gone and the `khroma` metadata exception is now explicitly non-blocking, but the reviewed `lodash-es` production `high` advisory still blocks the first trusted release until the shipped graph is upgraded.               |
...
| `pnpm lint`                         | fail                          | Prettier reports tracked-file formatting drift and still cannot parse committed pseudo-JSON pack-smoke artifacts whose first lines are command banners rather than JSON.                          |
...
| `pnpm verify:dependency-policy`     | pass with approved exceptions | Reported `1` approved production `high` advisory, `19` approved development `high` advisories, and `1` approved production license metadata exception after the `entities` exception was removed. |
...
6. The remaining blocking `lodash-es` production advisory is removed from the shipped graph, or a later reviewed decision explicitly reclassifies it as non-blocking for the first trusted release.
